...
const EditMap = () => {
    ...
    // Fetch categories and map data on component mount
    useEffect(() => {
        fetchCategories();
        fetchMapData();
    }, [mapId]);
    ...
    const fetchMapData = async () => {
        try {
            const response = await axios.get(import.meta.env.NODE_ENV==='production' ? import.meta.env.VITE_BACKEND_PROD : import.meta.env.VITE_BACKEND_DEV + `/api/cityMap/fetchMapDataById/${mapId}`);
            if (response.data && response.data.length > 0) {
                const mapData = response.data[0];
                setFormData({
                    map_city: mapData.map_city || '',
                    map_state: mapData.map_state || '',
                    map_category: mapData.map_category || '',
                    map_sub_category: mapData.map_sub_category || ''
                });
                setExistingImage(mapData.map_image || '');
            }
            setIsLoading(false);
        } catch (error) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (mapImage && mapImage.error) {
                alert("Please fix image errors before submitting");
                setFormSubmit(false);
                return;
            }

            // Check if sub-category already exists for this city and category (excluding current map)
            ...
            try {
                ...
                // Filter out the current map being edited
                const existingMaps = checkResponse.data.filter(map => map.map_id != mapId);
            ...
            formDataToSend.append('map_id', mapId);
            
            // Add image if new one is selected, otherwise keep existing
            if (mapImage && mapImage.file) {
                formDataToSend.append('image', mapImage.file);
            } else {
                formDataToSend.append('map_image', existingImage);
            }

            // Submit the form
            const response = await axios.put(
                import.meta.env.NODE_ENV==='production' ? import.meta.env.VITE_BACKEND_PROD : import.meta.env.VITE_BACKEND_DEV + '/api/cityMap/updateMap',
                formDataToSend,
                    ...
                    <div className="modal-backdrop">
                        <div className="modern-modal">
                            ...
                            <div className="modal-content">
                                ...
                                <div className="modal-subtext">City map has been updated successfully</div>
            ...
            <div className='main-wrapper'>
                <div className='row myproperty-section'>
                    ...
                    <div className="col-md-12 inside-section-wrapper">
                        <label className="myproperty-label">Map Image</label>
                        <input
                            type="file"
                            className="myproperty-location-input"
                        ...
                        <div className="image-upload-note" style={{fontSize: '0.85em', color: '#666', marginTop: 4}}>
                            (JPG, PNG, WEBP, 10KB - 5MB) - Leave empty to keep existing image
                        </div>
                        
                        {/* Show existing image or new image preview */}
                        {(existingImage || mapImage) && (
                            <div style={{marginTop: 8, position: 'relative', display: 'inline-block'}}>
                                <img
                                    src={
                                        mapImage 
                                            ? mapImage.preview 
                                            : `${import.meta.env.NODE_ENV==='production' ? import.meta.env.VITE_BACKEND_PROD : import.meta.env.VITE_BACKEND_DEV}/mapImages/${existingImage}`
                                    }
                                    alt="map preview"
                                    style={{ 
                                        width: 200, 
                                        ...
                                        objectFit: 'cover', 
                                        borderRadius: 6,
                                        border: mapImage && mapImage.error ? '2px solid #ec161e' : 'none'
                                    }}
                                />
                                {mapImage && (
                                    <button
